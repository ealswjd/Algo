...
public class Main {
    private static final int U = 0, D = 1, L = 2, R = 3; // 상하좌우
    private static final int[] dr = {-1, 1, 0, 0};
    private static final int[] dc = {0, 0, -1, 1};
    private static final int[] tl = {L, R, D, U}; // 좌회전
    private static final int[] tr = {R, L, U, D}; // 우회전
    private static int N, M, K; // 미로의 행 N, 열 M, 주문서 세트의 개수 K
        ...
        while(!q.isEmpty()) {
            ...
            if(left > 0) {
                ...
                for(int i=1; i<=cnt; i++) {
                    ...
                    if(rangeCheck(nr, nc) && !visited[nr][nc][left-i][right]) {
                        q.offer(new int[] {nr, nc, left-i, right});
                        visited[nr][nc][left-i][right] = true;
                    }
                }
            }
            // 우회전
            if(right > 0) {
                ...
                for(int i=1; i<=cnt; i++) {
                    ...
                    if(rangeCheck(nr, nc) && !visited[nr][nc][left][right-i]) {
                        q.offer(new int[] {nr, nc, left, right-i});
                        visited[nr][nc][left][right-i] = true;
                    }
                }
            }
        }
