...
public class Main {
	...
	private static Position bfs(int[] r, int[] b) {
		...
		q.offer(new Position(r[0], r[1], b[0], b[1], 0, new int[11]));
		
		Position cur;
		while(!q.isEmpty()) {
			cur = q.poll();
		
			if(cur.redR == or && cur.redC== oc) return cur;
			if(cur.cnt == 10 || (cur.blueR == or && cur.blueC == oc)) continue;
	...
	private static Position move(int r1, int c1, int r2, int c2, char c, int dir, Position cur, boolean isRed) {
		...
		if(!isRed) return new Position(r2, c2, r1, c1, cur.cnt+1, Arrays.copyOf(cur.order, 11));
		return new Position(r1, c1, r2, c2, cur.cnt+1, Arrays.copyOf(cur.order, 11));
	}//move
	...
	static class Position {
		...
		int cnt; // 움직인 횟수
		int[] order; // 움직인 방법
		public Position(int redR, int redC, int blueR, int blueC, int cnt, int[] order) {
			super();
			this.redR = redR;
			this.redC = redC;
			this.blueR = blueR;
