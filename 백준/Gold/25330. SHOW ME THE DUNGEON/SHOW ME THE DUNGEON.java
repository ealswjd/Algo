...
public class Main {
    private static final int A = 0, P = 1;

    private static int N, K; // 몬스터의 수, 초기 체력
    private static int max; // 해방시킬 수 있는 주민들의 최대 수
    private static int[][] info; // 각 마을에 있는 몬스터의 공격력, 주민의 수
    ...
    static void sol(int cur, int damage, int prev, int people) {
        ...
        for(int i=cur; i<N; i++) {
            int sum = prev + info[i][A]; // 이전 공격들 합 + 현재 공격
            int nDamage = damage + sum; // 지금까지 입은 공격 + 이번 마을에서의 공격
            int nPeople = people + info[i][P]; // 구할 수 있는 주민의 수
    ...
    private static void init() throws IOException {
        ...
        info = new int[N][2]; // 각 마을의 공격력, 주민의 수
        ...
        Arrays.sort(info, Comparator.comparingInt(o -> o[A]));
    }//init
