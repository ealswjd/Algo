...
public class Solution_2105_디저트카페 {
	...
	public static void main(String[] args) throws Exception {
		...
		for(int tc=1; tc<=T; tc++) {
			...
			ans = -1;
			for(startR=0; startR<N-2; startR++) {
				for(startC=0; startC<N-1; startC++) {
					dfs(0, startR, startC, new int[101], new boolean[N][N], 0);
				}
			}
			
			// 디저트를 가장 많이 먹을 때의 디저트 수 출력
	...
	private static void dfs(int n, int row, int col, int[] kind, boolean[][] visited, int cnt) {
		if(n > 3) return;
		if(n==3 && row==startR && col==startC) {
			if(cnt > ans) ans = cnt;
			return;
		}
		...
		for(int i=n; i<n+2; i++) {
			int nr = row + dr[i];
			int nc = col + dc[i];
			if(nr < 0 || nr >= N || nc < 0 || nc >= N || visited[nr][nc] || kind[map[nr][nc]] == 1) continue;
			visited[nr][nc] = true;
			kind[map[nr][nc]] = 1;
			dfs(i, nr, nc, kind, visited, cnt+1);
			visited[nr][nc] = false;
			kind[map[nr][nc]] = 0;
		}
